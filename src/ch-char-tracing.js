...
const ChineseCharacterTracing = () => {
  ...
  const drawBaseCharacter = () => {
    ...
    context.fillText(characters[currentIndex].char, 25, 250);
    
    templateContext.font = '250px SimSun';
    templateContext.fillStyle = 'rgba(0, 0, 0, 1)';
    templateContext.fillText(characters[currentIndex].char, 25, 250);
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-lg">
      ...
      <div className="relative" style={{ width: '300px', height: '300px', touchAction: 'none' }}>
        <canvas
          ref={canvasRef}
      ...
      <div className="mt-2">Completion: {completionPercentage.toFixed(2)}%</div>
      {showCelebration && (
        <div className="mt-4 bg-green-100 border-green-500 border-2 p-4 rounded-lg text-center">
          <Sparkles className="text-yellow-400 mx-auto" size={24} />
          <p className="text-xl font-bold mt-2">Great Job!</p>
          <p>You've completed the character!</p>
        </div>
      )}
